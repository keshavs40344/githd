export interface MasterShlokaTimestamp {
  chapter: number;
  verse: number;
  chapterName: string;
  videoId: string;
...
export function getMasterTimestampForVerse(chapter: number, verse: number): MasterShlokaTimestamp {
  const key = `${chapter}_${verse}`;
  if (GITA_MASTER_TIMESTAMPS[key]) {
    return GITA_MASTER_TIMESTAMPS[key];
  }
  // Default fallback to 1.1
  return GITA_MASTER_TIMESTAMPS["1_1"];
}
